...
// 리액트에서는 부모 컴포넌트가 자식 컴포넌트의 상태를 직접 변경할 수 있다.
// -> 자식에서 발생한 이벤트를 부모에수 처리하도록 하는 상태 끌어올리기 패턴을 이용함
...
const Id = (props) => {     // 부모에게서 전달받은 속성을 props 라고 부름
    ...
    return (
        // JSX 를 사용한 html 구문 작성부는 무조건 최상위 부모 태그 하나로 감싸져 있어야 한다.
        // JSX 식에는 부모 요소가 하나 있어야 합니다.(빈 태그도 가능)
        
        <>
            <div>
                <label>ID : </label>
                <input onChange={onChangeId}/>
                {/* onChange 이벤트 핸들러에 onChangeId 를 연결함 */}
            </div>
        </>
...
// Pw 컴포넌트 (자식)
const Pw = ({ onChangePw }) => {       // {} 이용하여 props 안에 있는 값을 바로 꺼내서 쓸 수도 있다.
...
const Exam3 = () => {
    ...
    // 자식의 상태를 부모에서 정의 (상태 끌어올리기 (자식 -> 부모))
