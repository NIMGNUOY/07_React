...
/*
    ...
    - 함수형 컴포넌트에 비해서 코드가 길다
    - Hooks 도입 이후 사용량이 감소되었다.

    2. 함수형 컴포넌트 : 함수형태로 구현
    <장점>
    - 간결함 : 코드 길이가 줄어들고, this 키워드 사용할 필요가 없어서 이해하기 쉽다.
    - Hooks 사용 가능 : Hooks 를 사용하여 함수형 컴포넌트에서도 상태관리와 
                       생명주기 메서드와 유사한 기능을 구현할 수 있음
    - 성능최적화 : 메모리 사용이 더 적고, 성능이 더 좋음
    ...
    2. state(상태) : 컴포넌트의 내부에서 관리되며 컴포넌트의 동작 및 UI 렌더링을
                     제어하는데 사용(내부 상태를 관리하는데 사용되는 데이터)
*/
...
class Exam1 extends Component { // 컴포넌트의 선언부
    ...
    /*
        ...
        - 변수는 컴포넌트 내에서 상태가 아닌 다른 데이터를 저장하기 위해 사용되므로 변수값이 변경되어도 컴포넌트는 다시 렌더링되지 않는다.
    */ 
    ...
    handleClick = () => {   // handleClick 이라는 이름의 함수를 정의함
        ...
        // 이 컴포넌트의 상태(state)중 count 를 현재 상태값에서 +1 한 값으로 상태를 다시 세팅(변경)
    ...
    render() {
        ...
        // return 에는 무엇을 렌더링할지 작성하면 됨. (jsx 문법으로 html 작성하면 됨)
...
export default Exam1;   // 컴포넌트 내보내기
                        // 해당 Exam1.js 파일에서 기본적으로 내보내는 컴포넌트의 이름을 Exam1